...
const RankingFinal = () => {
  ...
  // ✅ Agregados filtroNivel y filtroCategoria
  useEffect(() => {
    fetchRanking();
  }, [filtroGrupo, filtroZona, filtroTorneo, filtroNivel, filtroCategoria]);

  const cargarDatos = async () => {
    try {
      const [gruposRes, zonasRes, torneosRes, nivelesRes, categoriasRes] = await Promise.all([
        api.get('/grupos'),
        api.get('/zonas'),
        api.get('/torneos'),
        api.get('/niveles'),         // ✅ NUEVO
        api.get('/categorias')       // ✅ NUEVO
      ]);
      setGrupos(gruposRes.data);
      setZonas(zonasRes.data);
      setTorneos(torneosRes.data);
      setNiveles(nivelesRes.data);         // ✅ NUEVO
      setCategorias(categoriasRes.data);   // ✅ NUEVO
    } catch (err) {
  ...
  const fetchRanking = async () => {
    try {
      ...
      if (filtroNivel) url += `nivel_id=${filtroNivel}&`;           // ✅ NUEVO
      if (filtroCategoria) url += `categoria_id=${filtroCategoria}&`; // ✅ NUEVO
  ...
  const exportarAExcel = () => {
    ...
    const colWidths = [
      { wch: 10 }, { wch: 25 }, { wch: 25 }, { wch: 15 }, { wch: 15 },
      { wch: 15 }, { wch: 15 }, { wch: 10 }, { wch: 10 }, { wch: 10 }, { wch: 12 }, { wch: 10 }
    ];
...
const styles = {
  ...
  exportButton: { 
    backgroundColor: '#217346', color: '#ffffff', border: 'none', borderRadius: '8px', 
    padding: '12px 20px', fontSize: '1rem', fontWeight: '700', cursor: 'pointer',
